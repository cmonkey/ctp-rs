...
impl MdApi {
    pub fn CreateMdApiAndSpi(tx: Sender<MdSpiMsg>, flow_path: String, is_using_udp: bool, is_multicast: bool, _is_production_mode: bool) -> UniquePtr<MdApi> {
...
impl TraderApi {
    pub fn CreateTraderApiAndSpi(tx: Sender<TraderSpiMsg>, flow_path: String, _is_production_mode: bool) -> UniquePtr<TraderApi> {
        if !Path::new(&flow_path).exists() {
            create_dir_all(&flow_path).unwrap();
        }
